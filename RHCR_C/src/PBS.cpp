...
void PBS::find_conflicts(list<Conflict>& conflicts, int a1, int a2)
{
	...
	{ 
		...
		if (paths[a1]->size() != paths[a2]->size())
		{
			...
			int a2_ = paths[a1]->size() < paths[a2]->size() ? a2 : a1;//a1_路径短，a2_路径长
			int loc1 = paths[a1_]->back().location;
			for (size_t timestep = min_path_length; timestep < paths[a2_]->size(); timestep++)
			{
                //保证路径长的后续路径不与路径短的最后一个节点位置冲突
...
//针对以及重新生成后的路径，移除针对特定agv的冲突（已经被消解）
...
//选择最早的冲突，如果存在不可调和的冲突，则选择不可调和的冲突
void PBS::choose_conflict(PBSNode &node)
{
    ...
    for (auto conflict : node.conflicts)
    {
        ...
        if (std::get<4>(conflict) < std::get<4>(node.conflict))//get从0开始，get第5个元素，应该是冲突发生的时间
            node.conflict = conflict;
    }
    ...
    if (!nogood.empty())//作用？对比冲突表和nogood表，如果两AGV一致，则将此冲突设置为当前冲突
    {
        for (auto conflict : node.conflicts)
        {
            int a1 = std::get<0>(conflict);
...
double PBS::get_path_cost(const Path& path) const
{
    ...
    {
        double travel_time = 1;//在规划窗口内时，travel_time为1，超过规划窗口，会增加？
...
bool PBS::find_path(PBSNode* node, int agent)
{
    ...
    //压入新路径，需要两步操作吗？压入节点，以及压入PBS的paths中
...
//找出需要重新规划路径的agv，在所有冲突中，优先级低的agv
//注：优先级起始状态时，停留在起点的车辆拥有更高优先级
...
//找到与某个优先级相对应的一致路径，因为在优先级对中第一个agv优先级低，所以是调用时是first
bool PBS::find_consistent_paths(PBSNode* node, int agent)
{
    ...
    while (!replan.empty())
    {
        ...
        node->conflicts.splice(node->conflicts.end(), new_conflicts);//将new_conflicts剪切至conflicts.end()之后
    }
    runtime_find_consistent_paths += (double)(std::clock() - t) / CLOCKS_PER_SEC;
    if (screen == 2)
...
bool PBS::validate_consistence(const list<Conflict>& conflicts, const PriorityGraph &G)
{
    for (auto conflict : conflicts)
    {
        int a1 = std::get<0>(conflict);
        int a2 = std::get<1>(conflict);
        if (G.connected(a1, a2) || G.connected(a2, a1))//判断是否存在优先级关系
            return false;
    }
    return true;
}
...
bool PBS::generate_root_node()
{
    ...
    if (!lazyPriority)//lazyPriority的意思是？
    {
                    ...
                    int _time_limit)
{
	...
	while (!dfs.empty() && !solution_found)
	{
        ...
        for (auto & i : n)
                i = new PBSNode();
	    resolve_conflict(curr->conflict, n[0], n[1]);//表示产生两个CBS子节点
        ...
        for (auto & i : n)
        {
            ...
            if (sol)
            {
                HL_num_generated++;//扩展数和产生数的区别HL_num_expanded？
        ...
        if (!solution_found)//未找到合适的解，则先压入启发值大的节点，后压入启发值小的节点，应该是后压入的节点先弹出
        {
...
void PBS::get_solution()
{
    ...
    {
        solution[k] = *paths[k];//solution是mapf类成员，paths是pbs成员
    }

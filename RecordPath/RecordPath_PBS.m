function [PathSet,SystemTime] = RecordPath_PBS(RouteNow,Path_Length_Set,SystemTime,StepTime,ExpectedSteps,TaskGroup,TaskSetNum,SignTaskComplete,PathSet,CurrTempGoal)
...
Add_System = 0;
for i = 1:length(TaskSetNum)
    if Path_Length_Set(i) <= ExpectedSteps
        %记录已到达目的地的路径
        PathSet{i}((end+1):(end+Path_Length_Set(i)),:) = RouteNow(2:Path_Length_Set(i)+1,:,i);
        
        Add_System = max(Add_System,Path_Length_Set(i));
    else
        PathSet{i}((end+1):(end+ExpectedSteps),:) = RouteNow(2:ExpectedSteps+1,:,i);
        Add_System = max(Add_System,ExpectedSteps);
    end
end
SystemTime = SystemTime + Add_System*StepTime;
end
...
public:
    ...
    struct compare_node
    {
        // returns true if n1 > n2 (note -- this gives us *min*-heap).方法实现最小堆，启发值最小的点位于顶端
        bool operator()(const StateTimeAStarNode* n1, const StateTimeAStarNode* n2) const
        {
            if (n1->g_val + n1->h_val == n2->g_val + n2->h_val)
                return rand() % 2 == 0;  // break ties randomly，如果距离相等，则随机取一个点
            return n1->g_val + n1->h_val >= n2->g_val + n2->h_val;//不相等，则取较大？
        }
...
private:
	...
	fibonacci_heap< StateTimeAStarNode*, compare<StateTimeAStarNode::compare_node> > open_list; //采用斐波那契堆作为open_list的容器，能比较方便的实现查找最小值的功能

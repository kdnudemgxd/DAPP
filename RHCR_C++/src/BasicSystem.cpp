...
/// <summary>
/// 将求解器得到的解转移至basicsystem类中
/// </summary>
/// <param name="MAPF_paths"></param>
/// <param name="max_timestep"></param>
void BasicSystem::update_paths(const std::vector<Path>& MAPF_paths, int max_timestep = INT_MAX)
{
    for (int k = 0; k < num_of_drives; k++)
    {
...
void BasicSystem::update_initial_constraints(list< tuple<int, int, int> >& initial_constraints) const
{
    ...
    {
        int prev_location = -1;
        for (int t = timestep; t > max(0, timestep - k_robust); t--)//k_robust的作用是什么？
        {
            int curr_location = paths[k][t].location;
            if (curr_location < 0)
                continue;
            else if (curr_location != prev_location)
            {   //const导致无法修改成员变量，但可以修改所传入的参数；但此处t + k_robust + 1 - timestep的意义是什么？
...
//当在simulation_window窗口内保持位置不变的车辆超过1半，则判定为拥堵
...
list<tuple<int, int, int>> BasicSystem::move()
{
    ...
    for (int t = start_timestep; t <= end_timestep; t++)
    {
        for (int k = 0; k < num_of_drives; k++)
        {
            ...
            // check whether the move is valid，主要针对旋转代价
            if (t > 0)
            {
            ...
            // Check whether this move has conflicts with other agents，此处magic的意思到底是什么？
...
bool BasicSystem::solve()
{
	...
	else if (solver.get_name() == "WHCA")
	{
	 ...
	 else // PBS or ECBS
	 {
		 ...
		 {
			 sol = solver.run(starts, goal_locations, time_limit);
			 if (sol)
			 {
				 if (log)
					 solver.save_constraints_in_goal_node(outfile + "/goal_nodes/" + std::to_string(timestep) + ".gv");
				 update_paths(solver.solution);//更新路径到basicsystem的paths中
			 }
			 else
			 {
				 lra.resolve_conflicts(solver.solution);
...
void BasicSystem::initialize_solvers()
{
	...
	solver.initial_rt.hold_endpoints = true;//初始化时时false，现在改成true原因是什么？
...
bool BasicSystem::load_records()
{
	...
	for (int k = 0; k < num_of_drives; k++)//装载任务
	{
		getline(myfile, line);
		...
		timestep = min(timestep, paths[k].back().timestep);//此处装载的时间设置的意义是？
	}
	myfile.close();

...
int main(int argc, char** argv) 
{
	...
	desc.add_options()
		...
		("cutoffTime,t", po::value<int>()->default_value(200), "cutoff time (seconds)")
		...
		("simulation_time", po::value<int>()->default_value(100), "run simulation")
		...
		("hold_endpoints", po::value<bool>()->default_value(false),//暂时修改保持终点为true，测试是否能得到一致长的路径
		...
		("suboptimal_bound", po::value<double>()->default_value(1.5), "Suboptimal bound for ECBS")
	...
	input_buf << "map=maps/random-64-64-10.txt\nagentNum=90\nscenario=SORTING\nsimulation_window=50\nplanning_window=100\nsolver=PBS\nseed=0";
	//input_buf << "map=maps/DL_Comp_maps/DL_map_r_39_c_21.txt\nagentNum=90\nscenario=SORTING\nsimulation_window=50\nplanning_window=100\nsolver=ECBS\nseed=0";
	po::store(po::parse_config_file(input_buf, desc), vm);
	...
	else if (vm["scenario"].as<string>() == "SORTING")
	{
		 ...
		 set_parameters(system, vm);
		 G.preprocessing(system.consider_rotation);//装载启发值，如果文件不存在，则重新
		 system.simulate(vm["simulation_time"].as<int>());
		 return 0;
	}

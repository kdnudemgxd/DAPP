...
void ReservationTable::insertPath2CT(const Path& path)
{
	...
	while (curr != path.end() && curr->timestep - k_robust <= window)
	{
		...
		{
			if (G.types[prev->location] != "Magic")//magic是什么类型
...
//  insert the path to the conflict avoidance table，冲突避免表，用于软约束，应该是划分一个避免冲突的区间
...
// For PBS
//构建保留表，将所有高优先级的agv的路径作为约束加入约束表
// 添加初始约束？
//此外，如果使用了软约束，则同时添加冲突避免表
        ...
        const unordered_set<int>& high_priority_agents, int current_agent, int start_location)
{
    ...
    // add soft constraints，软约束具体指的是什么
